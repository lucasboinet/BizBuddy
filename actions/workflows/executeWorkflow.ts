...
import { ExecutionPhaseStatus, WorkflowExecutionPlan, WorkflowExecutionStatus, WorkflowExecutionTrigger, WorkflowStatus } from "@/types/workflows";
...
export async function ExecuteWorkflow(form: { workflowId: string, flowDefinition?: string }) {
  ...
  if (workflow.status === WorkflowStatus.PUBLISHED && !!workflow.executionPlan) {
  ...
  StartWorkflowExecution(execution.id, userId);
